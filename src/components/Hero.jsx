    ...
    <div className='xl:mx-44 md:mx-20 mt-14  md:mt-[20vh]'>
          <div className='gap-x-0 gap-y-10 flex flex-col-reverse md:flex-row'>
            <div className='flex flex-1 flex-col mx-5 mt-3'>
              ...
              <motion.p  initial={{ opacity: 0, x:-70 }} exit={{ opacity: 0, x:-30 }} whileInView={{ opacity: 1,x:0 }} transition={{ duration: 1, delay: 0.25 }} className='text-lg line-clamp-4 leading-normal mt-2 '>Hi, I'm <TextGradient text="Ghifari Nafhan" from="to-blue-500  hover:text-xl" via="via-violet-500" to="from-red-400" />. A Fullstack Developer from Indonesia. I'm Interested in Next JS, Nuxt JS, Nest JS, Express JS, Golang.</motion.p>
              <motion.div  initial={{ opacity: 0, y:70 }} exit={{ opacity: 0, y:30 }} whileInView={{ opacity: 1,y:0 }} transition={{ duration: 1, delay: 0.5 }}>
                <Button href="https://www.linkedin.com/in/gnafhan/" as={Link} className=' hover:bg-gradient-to-r hover:from-blue-500 hover:via-violet-500 hover:to-red-400 hover:bg-300% hover:animate-gradient mt-3 mr-4 text-white dark:text-black hover:scale-[105%] dark:hover:text-white font-semibold cursor-pointer' radius='sm' color='primary' startContent={<FaLinkedin/>} >Connect</Button>
                <Button href="/cv.pdf" as={Link}  className='mt-3 text-primary cursor-pointer hover:scale-[105%]' variant='bordered' radius='sm' color='primary' startContent={<FaArrowDown/>} >Resume</Button>
              </motion.div>
            </div>
